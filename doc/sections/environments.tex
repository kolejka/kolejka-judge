...
\begin{itemize}[label={}]
    ...
    \item \phantomsection \label{sec:run_steps} \docfunc{run_steps(self, steps)}
        \docfuncdesc{
            ...
            and a dictionary with execution statistics for each ran step. TODO CompletedProcess
        }
    ...
    \item \phantomsection \label{sec:run_command_step} \docfunc{run_command_step(self, step, name)}
        \docfuncdesc{
            ...
            \begin{itemize}
                ...
                \item evaluating the TODO \python{DependentExpr} expressions
    ...
    \item \docfunc{run_task_step(self, step, name)}
        \docfuncdesc{
            Responsible for running the task step, which consists of the following parts:
            \begin{itemize}
                \item verifying the prerequisites are met
                \item calling the TODO \python{execute()} method
            \end{itemize}
        }
    ...
    \item \docfunc{set_variable(self, variable_name, value)}
        \docfuncdesc{
            Used by the tasks to store any value that should be accessible by the command steps, while being
...
\begin{itemize}[label={}]
    \item \python{recognized_limits = ['cpu_affinity', 'time', 'memory']}
