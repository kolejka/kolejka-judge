...
class IOTask(TaskBase):
    ...
    @default_kwargs
    def __init__(self, executable, answer_path, generator_output_path, hinter_output_path, case_sensitive, space_sensitive, executable_arguments=None, input_path=None, hint_path=None, tool_override=None, tool_time=None, tool_c_standard=None, tool_cpp_standard=None, tool_gcc_arguments=None, tool_libraries=None, generator_source=None, verifier_source=None, hinter_source=None, checker_source=None, limit_cores=1, limit_time=None, limit_cpu_time=None, limit_real_time=None, limit_memory=None, limit_stack=None, limit_gpu_memory=None, limit_output_size=None, limit_error_size=None, **kwargs):
        ...
        self.tool_c_standard = tool_c_standard
        self.tool_cpp_standard = tool_cpp_standard
        self.tool_gcc_arguments = tool_gcc_arguments
...
class SingleIOTask(IOTask):
    ...
    def __init__(self, **kwargs):
        ...
        if self.generator_source:
            generator = GeneratorTask(source=self.generator_source, output_path=self.generator_output_path, override=self.tool_override, input_path=input_path, limit_real_time=self.tool_time, c_standard=self.tool_c_standard, cpp_standard=self.tool_cpp_standard, gcc_arguments=self.tool_gcc_arguments, libraries=self.tool_libraries)
            input_path = generator.output_path
            self.steps.append(('generator', generator))
        if self.verifier_source:
            verifier = VerifierTask(source=self.verifier_source, override=self.tool_override, input_path=input_path, limit_real_time=self.tool_time, c_standard=self.tool_c_standard, cpp_standard=self.tool_cpp_standard, gcc_arguments=self.tool_gcc_arguments, libraries=self.tool_libraries)
...
class MultipleIOTask(IOTask):
    ...
    def create_single(self, input_path):
        ...
        limit_stack = self.file_contents(self.alter_input_path(config.MULTITEST_INPUT_STACK, input_path))
        limit_stack = limit_stack and parse_memory(str(limit_stack, 'utf8').strip())
        limit_stack = limit_stack or self.limit_stack
